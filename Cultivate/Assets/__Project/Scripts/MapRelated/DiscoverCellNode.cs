...
public class DiscoverCellNode : CellNode
{
    ...
    [PortSettings(ShowBackingValue.Unconnected, ConnectionType.Override, TypeConstraint.None)] [SerializeField]
    private InputPort<SkillEntryCollectionDescriptor> Descriptor = new();
    ...
    protected override Cell CreateInternalCell()
    {
        ...
        if (descriptor == null)
        {
            Bound jingJieBound = new(JingJie.LianQi, preferredJingJie + 1);
            descriptor = new(pred: e => jingJieBound.Contains(e.LowestJingJie), count: 3);
        }
