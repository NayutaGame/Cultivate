...
public class GachaPanelDescriptor : PanelDescriptor
{
    ...
    public override void DefaultEnter(PanelDescriptor panelDescriptor)
    {
        ...
        List<SkillEntry> entries1 = RunManager.Instance.Environment.DrawSkills(new(
            pred: e => e.LowestJingJie <= JingJie.ZhuJi,
            count: 7,
            consume: false));
        
        List<SkillEntry> entries2 = RunManager.Instance.Environment.DrawSkills(new(
            pred: e => JingJie.JinDan <= e.LowestJingJie && e.LowestJingJie <= JingJie.YuanYing,
            count: 2,
            consume: false));
        
        List<SkillEntry> entries3 = RunManager.Instance.Environment.DrawSkills(new(
