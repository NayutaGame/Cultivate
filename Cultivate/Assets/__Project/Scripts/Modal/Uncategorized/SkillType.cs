
public class SkillType : SuperEnum<SkillType>
{
    private SkillType(int index, string name) : base(index, name) { }

    public static void Init()
    {
        _list = new SkillType[]
        {
            new(1 << 0, "攻击"),
            new(1 << 1, "灵气"),
            new(1 << 2, "二动"),
            new(1 << 3, "消耗"),
            new(1 << 4, "损耗"),
            new(1 << 5, "自指"),
            new(1 << 6, "治疗"),
        };
    }

    public static SkillType Attack   => _list[0];
    public static SkillType LingQi   => _list[1];
    public static SkillType ErDong   => _list[2];
    public static SkillType XiaoHao  => _list[3];
    public static SkillType SunHao   => _list[4];
    public static SkillType ZiZhi    => _list[5];
    public static SkillType ZhiLiao  => _list[6];

    public static implicit operator int(SkillType skillType) => skillType._index;
    // public static implicit operator SkillType(int index) => _list[index];
}
