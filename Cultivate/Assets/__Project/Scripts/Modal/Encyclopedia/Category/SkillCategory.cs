        ...
        AddRange(new List<SkillEntry>()
        {
            ...
            new(id:                         "0100",
                ...
                castDescription:            (j, dj, costResult, castResult) =>
                    ...
                    (j >= JingJie.JinDan ?
                        $"锋锐+{2 + dj}" :
                        $"初次：锋锐+{2 + dj}".ApplyCond(castResult)),
                withinPool:                 true,
                cast:                       async (env, caster, skill, recursive) =>
                {
                    await caster.GainArmorProcedure(2 + skill.Dj);
                    bool cond = skill.GetJingJie() >= JingJie.JinDan || skill.IsFirstTime || await caster.IsFocused();
                    if (cond)
            ...
            new(id:                         "0109",
                ...
                castDescription:            (j, dj, costResult, castResult) =>
                    $"{3 + dj}攻\n" +
                    $"击伤：锋锐+{2 + dj}".ApplyCond(castResult),
                cast:                       async (env, caster, skill, recursive) =>
                {
                    bool cond = false;
                    await caster.AttackProcedure(3 + skill.Dj,
                        damaged: async d =>
                        {
                            await caster.GainBuffProcedure("锋锐", 2 + skill.Dj);
                            cond = true;
                        },
                        wuXing: skill.Entry.WuXing);
                    return cond.ToCastResult();
            ...
            new(id:                         "0406",
                ...
                castDescription:            (j, dj, costResult, castResult) =>
                    $"消耗\n" +
                    $"回合开始时：灵气+1\n" +
                    $"唯一灵气牌（无法使用集中）：多1层".ApplyCond(castResult),
                cast:                       async (env, caster, skill, recursive) =>
                {
                    await skill.ExhaustProcedure();
            ...
            new(id:                         "0300",
                ...
                cast:                       async (env, caster, skill, recursive) =>
                {
                    ...
                    await caster.GainBuffProcedure("生命", 1 + 3 * skill.Dj);
            ...
            new(id:                         "0411",
                ...
                {
                    await skill.ExhaustProcedure();
                    await caster.AttackProcedure(10 + 10 * skill.Dj);
                    return null;
                }),
            ...
            new(id:                         "0404",
                ...
                cast:                       async (env, caster, skill, recursive) =>
                {
                    await caster.AttackProcedure(5 + skill.Dj, times: 3 + 3 * skill.Dj, wuXing: skill.Entry.WuXing);
                    return null;
                }),
            ...
            new(id:                         "0510",
                ...
                castDescription:            (j, dj, costResult, castResult) =>
                    $"架势：免除吟唱\n" +
                    $"护甲+{8 + 6 * dj}\n" +
                    (j >= JingJie.YuanYing ? $"无护甲：翻倍".ApplyCond(castResult) : ""),
                cast:                       async (env, caster, skill, recursive) =>
                {
                    if (skill.GetJingJie() >= JingJie.YuanYing)
                    {
            ...
            new(id:                         "0523",
                ...
                cast:                       async (env, caster, skill, recursive) =>
                {
                    ...
                    await caster.GainBuffProcedure("软弱", 2 + skill.Dj);
                    return null;
                }),
            ...
            new(id:                         "0500",
                ...
                castDescription:            (j, dj, costResult, castResult) =>
                    $"{12 + 4 * dj}攻" +
                    (j > JingJie.JinDan ? $"消耗所有灵气，每1：多5攻" : ""),
                cast:                       async (env, caster, skill, recursive) =>
                {
            ...
            new(id:                         "0509",
                ...
                cast:                       async (env, caster, skill, recursive) =>
                {
                    await caster.AttackProcedure((4 + skill.Dj) * (4 + skill.Dj), wuXing: skill.Entry.WuXing);
                    return null;
                }),
            ...
            new(id:                         "0524",
                ...
                cast:                       async (env, caster, skill, recursive) =>
                {
                    await caster.GainBuffProcedure("柔韧", 3 + 3 * skill.Dj);
                    await caster.GainBuffProcedure("内伤", 3 + 3 * skill.Dj);
                    return null;
                }),
