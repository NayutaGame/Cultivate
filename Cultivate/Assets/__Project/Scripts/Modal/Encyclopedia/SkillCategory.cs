    ...
    public SkillCategory()
    {
        AddRange(new List<SkillEntry>()
        {
            ...
            new("武曲", new CLLibrary.Range(3, 5),
                ...
                execute: async (caster, waiGong, recursive) =>
                {
                    int add = await caster.TryConsumeBuff("锋锐") ? (8 + 2 * waiGong.Dj) : 0;
            ...
            new("秋水", new CLLibrary.Range(2, 5), new SkillDescription((j, dj) => $"{12 + 2 * dj}攻\n消耗1锋锐：吸血\n充沛：翻倍"),
                ...
                execute: async (caster, waiGong, recursive) =>
                {
                    int d = await caster.TryConsumeMana() ? 2 : 1;
                    await caster.AttackProcedure((12 + 2 * waiGong.Dj) * d, lifeSteal: await caster.TryConsumeBuff("锋锐"),
                        wuXing: waiGong.Entry.WuXing);
                }),
            ...
            new("奔腾", new CLLibrary.Range(4, 5), new SkillDescription((j, dj) => $"二动\n充沛：三动"), WuXing.Shui,
                ...
                {
                    caster.Swift = true;
                    if (await caster.TryConsumeMana())
            ...
            new("身骑白马", new CLLibrary.Range(1, 5),
                ...
                execute: async (caster, waiGong, recursive) =>
                {
                    if (waiGong.StageUsedTimes < 2 + waiGong.Dj)
                        return;
                    await caster.AttackProcedure((6 + 2 * waiGong.Dj) * (6 + 2 * waiGong.Dj), pierce: true,
                        wuXing: waiGong.Entry.WuXing);
                }),
            ...
            new("千年笋", new CLLibrary.Range(2, 5), new SkillDescription((j, dj) => $"{15 + 3 * dj}攻\n消耗1格挡：穿透"),
                ...
                execute: async (caster, waiGong, recursive) =>
                {
                    await caster.AttackProcedure(15 + 3 * waiGong.Dj, wuXing: waiGong.Entry.WuXing,
                        pierce: await caster.TryConsumeBuff("格挡"));
                }),
            ...
            new("亢龙有悔", new CLLibrary.Range(4, 5), "10攻x3 穿透\n闪避+3\n每使用一次效果减弱", WuXing.Mu,
                ...
                execute: async (caster, waiGong, recursive) =>
                {
                    ...
                    await caster.AttackProcedure(10, times: 3 - waiGong.RunUsedTimes, pierce: true,
                        wuXing: waiGong.Entry.WuXing);
            ...
            new("化劲", new CLLibrary.Range(2, 5), new SkillDescription((j, dj) => $"消耗5生命\n灼烧+{2 + dj}\n消耗1力量：翻倍"),
                ...
                execute: async (caster, waiGong, recursive) =>
                {
                    ...
                    int d = await caster.TryConsumeBuff("力量") ? 2 : 1;
            ...
            new("燃灯留烬", new CLLibrary.Range(3, 5),
                ...
                execute: async (caster, waiGong, recursive) =>
                {
                    await caster.ArmorGainSelfProcedure((caster.ExhaustedCount + 1) * (6 + 2 * waiGong.Dj));
                }),
            ...
            new("高速剑阵", new CLLibrary.Range(2, 5), new SkillDescription((j, dj) => $"灵气+{4 + dj}\n架势：二动"), WuXing.Tu,
                skillTypeCollection: SkillTypeCollection.JianZhen | SkillTypeCollection.LingQi,
                execute: async (caster, waiGong, recursive) =>
                {
