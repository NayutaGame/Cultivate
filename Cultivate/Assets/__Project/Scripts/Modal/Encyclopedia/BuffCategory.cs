    ...
    public BuffCategory()
    {
        AddRange(new List<BuffEntry>()
        {
            ...
            new("禁止治疗", "禁止治疗", BuffStackRule.Add, true, false,
                eventCaptures: new StageEventCapture[]
                {
                    new StageEnvironmentEventCapture(CLEventDict.WILL_HEAL, 0, async (listener, eventDetails) =>
                    {
                        Buff b = (Buff)listener;
            ...
            new("长明灯", "获得灵气时：每1，生命+3", BuffStackRule.Add, true, false,
                eventCaptures: new StageEventCapture[]
                {
                    new StageEnvironmentEventCapture(CLEventDict.DID_BUFF, 0, async (listener, eventDetails) =>
                    {
                        Buff b = (Buff)listener;
            ...
            new("尖刺陷阱", "下次受到攻击时，对对方施加等量减甲", BuffStackRule.Add, true, false,
                eventCaptures: new StageEventCapture[]
                {
                    new StageEnvironmentEventCapture(CLEventDict.DID_ATTACK, 0, async (listener, eventDetails) =>
                    {
                        Buff b = (Buff)listener;
            ...
            new("回合力量", "每回合：力量+[层数]", BuffStackRule.Add, true, false,
                eventCaptures: new StageEventCapture[]
                {
                    new StageEnvironmentEventCapture(CLEventDict.START_TURN, 0, async (listener, eventDetails) =>
                    {
                        Buff b = (Buff)listener;
            ...
            new("回合免疫", "此回合无法收到伤害", BuffStackRule.One, true, false,
                eventCaptures: new StageEventCapture[]
                {
                    ...
                    new StageEnvironmentEventCapture(CLEventDict.WILL_DAMAGE, 0, async (listener, eventDetails) =>
                    {
                        Buff b = (Buff)listener;
            ...
            new("外骨骼", "每次攻击前，护甲+3", BuffStackRule.One, true, false,
                eventCaptures: new StageEventCapture[]
                {
                    new StageEnvironmentEventCapture(CLEventDict.WILL_ATTACK, 0, async (listener, eventDetails) =>
                    {
                        Buff b = (Buff)listener;
                        AttackDetails d = (AttackDetails)eventDetails;
            ...
            new("火箭靴", "使用灵气牌时，获得二动", BuffStackRule.One, true, false,
                eventCaptures: new StageEventCapture[]
                {
                    new StageEnvironmentEventCapture(CLEventDict.END_STEP, 0, async (listener, eventDetails) =>
                    {
                        Buff b = (Buff)listener;
            ...
            new("定龙桩", "对方二动时，如果没有暴击，获得1", BuffStackRule.One, true, false,
                eventCaptures: new StageEventCapture[]
                {
                    new StageEnvironmentEventCapture(CLEventDict.DID_SWIFT, 0, async (listener, eventDetails) =>
                    {
                        Buff b = (Buff)listener;
            ...
            new("飞行器", "成功闪避时，如果对方没有跳回合，施加1", BuffStackRule.One, true, false,
                eventCaptures: new StageEventCapture[]
                {
                    new StageEnvironmentEventCapture(CLEventDict.DID_EVADE, 0, async (listener, eventDetails) =>
                    {
                        Buff b = (Buff)listener;
            ...
            new("时光机", "使用一张牌前，升级", BuffStackRule.One, true, false,
                eventCaptures: new StageEventCapture[]
                {
                    new StageEnvironmentEventCapture(CLEventDict.WILL_EXECUTE, 0, async (listener, eventDetails) =>
                    {
                        Buff b = (Buff)listener;
            ...
            new("缠绕", "无法二动/三动\n每回合：层数-1", BuffStackRule.Add, false, true,
                eventCaptures: new StageEventCapture[]
                {
                    new StageEnvironmentEventCapture(CLEventDict.WILL_SWIFT, 0, async (listener, stageEventDetails) =>
                    {
                        Buff b = (Buff)listener;
