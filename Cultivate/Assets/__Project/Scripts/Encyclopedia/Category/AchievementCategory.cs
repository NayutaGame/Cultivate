            ...
            new(id: "ACH001",
                ...
                runClosures: new RunClosure[]
                {
                    new(RunClosureDict.DID_COMMIT_RUN, 0, async (owner, closure, details) => {
                        ...
                        if (p.IsUnlocked()) return;
                        if (d.RunEnvironment.GetRunConfig().GetCharacter() != Encyclopedia.CharacterCategory.FromName("徐福")) return;
            ...
            new(id: "ACH008",
                ...
                runClosures: new RunClosure[]
                {
                    new(RunClosureDict.DID_COMMIT_RUN, 0, async (owner, closure, details) => {
                        ...
                        if (p.IsUnlocked()) return;
                        if (d.RunEnvironment.GetRunConfig().GetCharacter() != Encyclopedia.CharacterCategory.FromName("子非鱼")) return;
                        if (d.RunEnvironment.GetRunConfig().GetDifficulty() < 8) return;
                        if (d.RunEnvironment.GetResult().GetOutcome() != RunResult.RunOutcome.Victorious) return;
            ...
            new(id: "ACH015",
                ...
                runClosures: new RunClosure[]
                {
                    new(RunClosureDict.DID_COMMIT_RUN, 0, async (owner, closure, details) => {
                        ...
                        if (p.IsUnlocked()) return;
                        if (d.RunEnvironment.GetRunConfig().GetCharacter() != Encyclopedia.CharacterCategory.FromName("子非燕")) return;
                        if (d.RunEnvironment.GetRunConfig().GetDifficulty() < 8) return;
                        if (d.RunEnvironment.GetResult().GetOutcome() != RunResult.RunOutcome.Victorious) return;
            ...
            new(id: "ACH019",
                ...
                stageClosures: new StageClosure[]
                {
                    ...
                    new(StageClosureDict.DID_GAIN_BUFF, 0, async (owner, closure, details) => {
                        ...
                        WuXing wuXing = WuXing.TraversalBasic.FirstObj(wuXing => wuXing.GetElementaryBuff() == d.BuffEntry);
                        if (wuXing == null)
                            return;

                        int stack = home.GetStackOfBuff(d.BuffEntry);

                        const string MAX_WUXING_BUFF_KEY = "MaxWuXingBuffStack";
                        home.Memory.PerformOperation(MAX_WUXING_BUFF_KEY, 0, current => Math.Max(current, stack));
                    }),
            ...
            new(id: "ACH022",
                ...
                runClosures: new RunClosure[]
                {
                    new(RunClosureDict.DID_COMMIT_RUN, 0, async (owner, closure, details) => {
                        ...
                        if (p.IsUnlocked()) return;
                        if (d.RunEnvironment.GetRunConfig().GetCharacter() != Encyclopedia.CharacterCategory.FromName("彼此卿")) return;
                        if (d.RunEnvironment.GetRunConfig().GetDifficulty() < 8) return;
                        if (d.RunEnvironment.GetResult().GetOutcome() != RunResult.RunOutcome.Victorious) return;
            ...
            new(id: "ACH028",
                ...
                stageClosures: new StageClosure[]
                {
                    ...
                    new(StageClosureDict.DID_COMMIT, 0, async (owner, closure, details) => {
                        ...
                        if (skill.GetEntry() != Encyclopedia.SkillCategory.FromName("幻化")) return;
            ...
            new(id: "ACH029",
                ...
                runClosures: new RunClosure[]
                {
                    new(RunClosureDict.DID_COMMIT_RUN, 0, async (owner, closure, details) => {
                        ...
                        if (p.IsUnlocked()) return;
                        if (d.RunEnvironment.GetRunConfig().GetCharacter() != Encyclopedia.CharacterCategory.FromName("风雨晴")) return;
                        if (d.RunEnvironment.GetRunConfig().GetDifficulty() < 8) return;
                        if (d.RunEnvironment.GetResult().GetOutcome() != RunResult.RunOutcome.Victorious) return;
                ...
                runClosures: new RunClosure[]
                {
                    new(RunClosureDict.WIL_CHANGE_CELL, 0, async (owner, closure, details) => {
                        ...
                        int noAttributeCardCount = RunManager.Instance.Environment
                            .TraversalSkills()
                            .Count(skill => !skill.GetEntry().GetWuXing().IsBasic());
